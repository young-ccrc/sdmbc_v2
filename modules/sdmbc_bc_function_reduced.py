...
def correct_daily_mean(
    ...
    for i in range(iyr):
        for j in range(nout):
            jd = 29 if j == 1 and is_leap_year(iyr + startyear_h) else days_in_month[j]

            # Modify ggd in place to avoid unnecessary memory usage
            ggd[:, i, j, :jd] = (gd[:, i, j, :jd] - avdc[:, j, :jd]) + avdh[:, j, :jd]

    # Modify ggd based on phul and phll conditions
    ggd = np.clip(ggd, phll[jj, :, None, None, None], phul[jj, :, None, None, None])

    avdc_iter[itr, :, :, :] = avdc.copy()

    return ggd, avdc_iter


def correct_daily_mean_sd(
    ggd,
    sddh,
    phul,
    phll,
    ...
    # Precompute sddc and avd using the Fortran function
    avd, sddc, cordc = mbc.sdsmooth(
        ggd, moving_window, nday, nsc, 3, leap, idays, missing_value
    )
    del cordc
    # Calculate fact in place to minimize memory usage
    ...
    for i in range(iyr):
        for j in range(nout):
            ...
            # Modify ggd in place to apply corrections
            ggd[:, i, j, :jd] = (ggd[:, i, j, :jd] - avd[:, j, :jd]) * fact[
                :, j, :jd
            ] + avd[:, j, :jd]

    # Clip ggd based on phul and phll
    ggd = np.clip(ggd, phll[jj, :, None, None, None], phul[jj, :, None, None, None])

    sddc_iter[itr, :, :, :] = sddc.copy()

    return ggd, sddc_iter


def correct_daily_correlation(
    nvar,
    ggd,
    gdct,
    dobs,
    ...
    # Precompute sdd and cmod/gmod using the Fortran functions
    avd, sdd, cord = mbc.sdsmooth(
        ggd, moving_window, nday, ns1, jj + 1, leap, idays, missing_value
    )
    del cord
    cmod, gmod = mbc.c_g_corl_daily(
        ggd, moving_window, nday, ns1, jj + 1, leap, idays, 2, missing_value
    )
    ...
    for i in range(iyr):
        for j in range(nout):
            ...
            for day_index in range(jd):
                ...
                bt[:nvar] = (
                    ggd[:nvar, i, j, day_index] - avd[:nvar, j, day_index]
                ...
                # Perform matrix multiplications outside the innermost loop
                temp = np.matmul(go, gmg)
                temp1 = np.matmul(temp, bt)
                temp2 = np.matmul(temp, cmg)
                temp3 = np.matmul(temp2, btprev)

                gcur[:nvar] = np.matmul(co, gprev) + temp1 - temp3

                gprev[:nvar] = gcur[:nvar]
                btprev[:nvar] = bt[:nvar]

                # Update gdct in place
                gdct[:nvar, i, j, day_index] = np.clip(
                    gcur[:nvar] * sdd[:nvar, j, day_index] + avd[:nvar, j, day_index],
                    phll[jj, :nvar],
                    phul[jj, :nvar],
                )

    return gdct, cmod_iter, gmod_iter


def correct_monthly_mean(ggm, gm, avmh, phul, phll, jj, itr, avmc_iter, tprint=0):
    if tprint == 1 and itr > 0:
        print("Correcting for monthly mean", jj)

    # Calculate mean and sd of gcm monthly series, historical
    avmc, sdmc, cormc = mbc.avsds(gm)
    del sdmc, cormc
    avmc_iter[itr, :, :] = avmc.copy()

    # Calculate new values for ggm in place
    ggm = (gm - avmc[:, None, :]) + avmh[:, None, :]

    # Clip ggm based on phul and phll
    ggm = np.clip(ggm, phll[jj, :, None, None], phul[jj, :, None, None])

    return ggm, avmc_iter


def correct_monthly_mean_sd(ggm, sdmh, phul, phll, jj, itr, sdmc_iter, tprint=0):
    if tprint == 1:
        print("Correcting for monthly mean and standard deviation", jj)

    # Calculate mean and sd of gcm mean-corrected monthly series, historical
    avm, sdmc, cormc = mbc.avsds(ggm)
    del cormc
    sdmc_iter[itr, :, :] = sdmc.copy()

    # Calculate factor for sd correction
    ...
    ggm = (ggm - avm[:, None, :]) * fact[:, None, :] + avm[:, None, :]

    # Clip corrected ggm in place
    ggm = np.clip(ggm, phll[jj, :, None, None], phul[jj, :, None, None])

    return ggm, sdmc_iter


def correct_monthly_correlation(
    nvar,
    ggm,
    gmct,
    dobsm,
    ...
    del corm
    # Get correlation modulations
    cmodm, gmodm = mbc.c_g_corl_season(ggm, 2)
    ...
    for i in range(iyr):
        for j in range(nout):
            ...
            bt[:nvar] = (ggm[:nvar, i, j] - avm[:nvar, j]) / sdm[:nvar, j]

            if i == 0 and j == 0:
                gprev[:nvar] = bt[:nvar]
                btprev[:nvar] = bt[:nvar]

            # Matrix multiplication, move repeated calculations outside
            temp = np.matmul(go, gmg)
            temp1 = np.matmul(temp, bt)
    ...
    return gmct, cmodm_iter, gmodm_iter


def correct_seasonal_mean(ggs, gs, avsh, phul, phll, jj, itr, avsc_iter, tprint=0):
    if tprint == 1 and itr > 0:
        print("Correcting for seasonal mean", jj)

    # Calculate mean and sd of gcm seasonal series, historical
    avsc, sdsc, corsc = mbc.avsds(gs)
    del sdsc, corsc
    avsc_iter[itr, :, :] = avsc.copy()

    # Calculate new values for ggs in place
    ggs = (gs - avsc[:, None]) + avsh[:, None]

    # Clip ggs based on phul and phll
    ggs = np.clip(ggs, phll[jj, :, None], phul[jj, :, None])

    return ggs, avsc_iter


def correct_seasonal_mean_sd(ggs, sdsh, phul, phll, jj, itr, sdsc_iter, tprint=0):
    if tprint == 1:
        print("Correcting for seasonal mean and standard deviation", jj)

    # Calculate mean and sd of gcm mean-corrected seasonal series, historical
    ...
    # Correct for bias in seasonal sd
    ggs = (ggs - avs[:, None]) * fact[:, None] + avs[:, None]

    # Clip corrected ggs in place
    ggs = np.clip(ggs, phll[jj, :, None], phul[jj, :, None])

    return ggs, sdsc_iter


def correct_seasonal_correlation(
    nvar,
    ggs,
    gsct,
    dobss,
    ...
    # Calculate correlation modulations
    avs, sds, cors = mbc.avsds(ggs)
    del cors
    cmods, gmods = mbc.c_g_corl_season(ggs, 2)
    ...
    for i in range(iyr):
        for iss in range(nss):
            ...
            bt[:nvar] = (ggs[:nvar, i, iss] - avs[:nvar, iss]) / sds[:nvar, iss]

            if i == 0 and iss == 0:
                gprev[:nvar] = bt[:nvar]
                btprev[:nvar] = bt[:nvar]

            # Perform matrix operations for correlation adjustments
            temp = np.matmul(go, gmg)
            temp1 = np.matmul(temp, bt)
            temp2 = np.matmul(np.matmul(temp, cmg), btprev)
            temp4 = np.matmul(co, gprev)

            gcur[:nvar] = temp4[:nvar] + temp1[:nvar] - temp2[:nvar]

            gprev[:nvar] = gcur[:nvar]
            btprev[:nvar] = bt[:nvar]

            # Apply corrections to gsct
            gsct[:nvar, i, iss] = np.clip(
                gcur[:nvar] * sds[:nvar, iss] + avs[:nvar, iss],
                phll[jj, :nvar],
                phul[jj, :nvar],
            )

    return gsct, cmods_iter, gmods_iter


def correct_annual_mean(ggy, gy, avyh, phul, phll, jj, itr, avyc_iter, tprint=0):
    if tprint == 1 and itr > 0:
        print("Correcting for annual mean", jj)

    # Calculate mean and sd of gcm annual series, historical
    avyc, sdsy, coryc = mbc.avsdy(gy)
    del sdsy, coryc
    avyc_iter[itr, :] = avyc.copy()

    # Calculate new values for ggy in place
    ggy = (gy - avyc[:, None]) + avyh[:, None]

    # Clip ggy based on phul and phll
    ggy = np.clip(ggy, phll[jj, :, None], phul[jj, :, None])

    return ggy, avyc_iter


def correct_annual_mean_sd(ggy, sdyh, phul, phll, jj, itr, sdyc_iter, tprint=0):
    if tprint == 1:
        print("Correcting for annual mean and standard deviation", jj)

    # Calculate mean and sd of gcm mean-corrected annual series, historical
    ...
    # Correct for bias in annual sd
    ggy = (ggy - avy[:, None]) * fact[:, None] + avy[:, None]

    # Clip corrected ggy in place
    ggy = np.clip(ggy, phll[jj, :, None], phul[jj, :, None])

    return ggy, sdyc_iter


def correct_annual_correlation(
    nvar,
    ggy,
    gyct,
    dobsy,
    ...
    for i in range(iyr):
        ...
        # Apply corrections to gyct
        gyct[:nvar, i] = np.clip(
            gcur[:nvar] * sdy[:nvar] + avy[:nvar], phll[jj, :nvar], phul[jj, :nvar]
        )

    return gyct, cmody_iter, gmody_iter
...
def bc_correction_hist(gcm_reshape, obs_reshape):
    ...
    del rem, res, rey, cordh, cormh, corsh, coryh, obs_reshape
    ...
    for itr in range(nntr):
        ...
        if itr == 0:
            ...
            potential_phul = gd * ac + 3 * sddh[:, np.newaxis, :, :]
            ...
            potential_phll = gd * ac - 3 * sddh[:, np.newaxis, :, :]
            ...
            if irho[0, 0] != 0:  # hist: daily mean, future: - avdc + avdh
                ...
                ggd, avdc_iter = correct_daily_mean(
                    ggd,
                    gd,
                    avdc,
                    avdh,
            ...
            if irho[0, 1] != 0:  # hist: daily sd, future: (x - avd)*sddh/sddc + avd
                ...
                ggd, sddc_iter = correct_daily_mean_sd(
                    ggd,
                    sddh,
                    phul,
                    phll,
            ...
            if (
                ...
                if itr == nntr - 1 and nxt > 0:
                    ...
                    gdct, cmod_iter, gmod_iter = correct_daily_correlation(
                        nvar,
                        ggd,
                        gdct,
                        dobs,
            ...
            for k in range(nvar):
                for i in range(iyr):
                    for j in range(nout):
                        ...
                        ggm[k, i, j] = np.sum(gdct[k, i, j, :days]) / days

            gmct = ggm.copy()

            if irho[1, 0] != 0:  # hist: monthly mean, future: - avmc + avmh
                ggm, avmc_iter = correct_monthly_mean(
                    ggm, gm, avmh, phul, phll, jj, itr, avmc_iter, tprint
                )
                gmct = ggm.copy()
            ...
            if (
                ...
                ggm, sdmc_iter = correct_monthly_mean_sd(
                    ggm, sdmh, phul, phll, jj, itr, sdmc_iter, tprint
                )
                gmct = ggm.copy()
            ...
            if (
                ...
                if itr == nntr - 1 and nxt > 0:
                    ...
                    gmct, cmodm_iter, gmodm_iter = correct_monthly_correlation(
                        nvar,
                        ggm,
                        gmct,
                        dobsm,
            ...
            if irho[2, 0] != 0:  # hist: seasonal mean, future: - avsc + avsh
                ggs, avsc_iter = correct_seasonal_mean(
                    ggs, gs, avsh, phul, phll, jj, itr, avsc_iter, tprint
                )
                gsct = ggs.copy()
            ...
            if (
                ...
                ggs, sdsc_iter = correct_seasonal_mean_sd(
                    ggs, sdsh, phul, phll, jj, itr, sdsc_iter, tprint
                )
                gsct = ggs.copy()
            ...
            if (
                ...
                gsct, cmods_iter, gmods_iter = correct_seasonal_correlation(
                    nvar,
                    ggs,
                    gsct,
                    dobss,
            ...
            if irho[3, 0] != 0:  # hist: annual mean, future: - avyc + avyh
                ggy, avyc_iter = correct_annual_mean(
                    ggy, gy, avyh, phul, phll, jj, itr, avyc_iter, tprint
                )
                gyct = ggy.copy()
            ...
            if (
                ...
                ggy, sdyc_iter = correct_annual_mean_sd(
                    ggy, sdyh, phul, phll, jj, itr, sdyc_iter, tprint
                )
                gyct = ggy.copy()

            if (
                irho[3, 2] != 0 or irho[3, 3] != 0
            ):  # hist: annual corl, future: cmody, gmody, cobsy, dobsy
                gyct, cmody_iter, gmody_iter = correct_annual_correlation(
                    nvar,
                    ggy,
                    gyct,
                    dobsy,
                    ...
                    for day_index in range(jd):
                        for k in range(nvar):
                            ...
                            if aa > missing_value:
                                ...
                                if time_scale == 0 and gd[k, i, j, day_index] != 0.0:
                                    af1 = (
                                ...
                                ac1 = aa * ac
                                ac1 = np.clip(ac1, phll[jj, k], phul[jj, k])
