...
case "$KIND" in
    3d)
        ...
        WATCHDOG_MINUTES=${WATCHDOG_MINUTES_OVERRIDE:-60}
...
qsub \
    ...
    -l ncpus="$NCPUS" \
    -l jobfs="${JOBFS_GB}GB" \
    -v CONFIG="$CONFIG",VAR="$VAR",NCPUS="$NCPUS",MEM_GB="$MEM_GB",MODULE_ANALYSIS="$MODULE_ANALYSIS",SCRIPT_PATH="$PYTHON_SCRIPT",PYTHON_ARGS="$PYTHON_ARGS",WATCHDOG_MINUTES="$WATCHDOG_MINUTES" \
    "$PBS_SCRIPT"
